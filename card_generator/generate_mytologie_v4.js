const puppeteer = require('puppeteer');
const fs = require('fs');
const path = require('path');

// Paths
const DATA_FILE = path.join(__dirname, 'mytologie_data.json');
const TEMPLATE_FILE = path.join(__dirname, 'mytologie_template_v4.html');
const IMG_SRC_DIR = path.join(__dirname, '../kvarteta-eshop/public/cards/mytologie');
const OUTPUT_DIR = path.join(__dirname, '../kvarteta-eshop/public/cards/mytologie_v4');

// Vytvoření složky, pokud neexistuje (podle přání uživatele "nic nemaž a udělej novou složku")
if (!fs.existsSync(OUTPUT_DIR)) {
    fs.mkdirSync(OUTPUT_DIR, { recursive: true });
}

async function run() {
    const data = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
    const template = fs.readFileSync(TEMPLATE_FILE, 'utf8');

    const browser = await puppeteer.launch({
        args: ['--allow-file-access-from-files', '--disable-web-security']
    });
    const page = await browser.newPage();
    // Using original image proportions (678x921px)
    await page.setViewport({ width: 678, height: 921, deviceScaleFactor: 2 });
    await page.setDefaultNavigationTimeout(60000);

    console.log(`Starting generation of ${data.length} mythology cards in V4 style (Print format + Czech grammar)...`);

    for (const char of data) {
        let html = template;

        // Originální obrázek
        let srcFileName = char.img;
        
        // Pokud originální obrázek obsahuje "_karta_", použijeme ten. Pokud ne, nebo se jedná o původní asset.
        const imgSrc = path.join(IMG_SRC_DIR, srcFileName);
        
        if (!fs.existsSync(imgSrc)) {
            console.error(`❌ Image not found: ${imgSrc}`);
            // Zkus nahradit za _karta_ kdyby chybělo
            continue;
        }
        
        const base64Img = fs.readFileSync(imgSrc).toString('base64');
        const imgUrl = `data:image/png;base64,${base64Img}`;

        // Kalkulace velikosti písma pro dlouhé názvy
        const fullTitle = `${char.id} - ${char.name}`;
        let titleSize = '38px';
        if (fullTitle.length > 20) titleSize = '28px';
        else if (fullTitle.length > 17) titleSize = '33px';

        // Replace placeholders
        html = html.replace(/\${TITLE_SIZE}/g, titleSize);
        html = html.replace(/\${ID}/g, char.id);
        
        // Zde VYNUCUJEME gramaticky správný název. Databáze obsahuje např. "Zeus". 
        // ID zůstává. Zkusíme to zachovat tak, jak je to v JSONu.
        html = html.replace(/\${NAME}/g, char.name);
        html = html.replace(/\${SUBTITLE}/g, char.subtitle);
        html = html.replace(/\${ORIGIN}/g, char.origin);
        html = html.replace(/\${MAGIE}/g, char.magic);
        html = html.replace(/\${STARI}/g, char.age);
        html = html.replace(/\${SILA}/g, char.strength);
        html = html.replace(/\${HROZIVOST}/g, char.menace);
        html = html.replace(/\${COLOR}/g, char.color);
        html = html.replace(/\${IMG_PATH}/g, imgUrl);

        // Render HTML
        await page.setContent(html, { waitUntil: 'load', timeout: 60000 });

        // Wait for fonts/effects
        await new Promise(r => setTimeout(r, 200));

        const element = await page.$('.card-container');
        
        // Vytvoření odlišeného názvu (v4 místo karta/v2)
        const outputFilename = char.img.replace('_karta_', '_v4_');
        const outputPath = path.join(OUTPUT_DIR, outputFilename);

        await element.screenshot({ path: outputPath, omitBackground: true });
        console.log(`✅ Generated (V4): ${char.name} -> ${path.basename(outputPath)}`);
    }

    await browser.close();
    console.log('--- ALL V4 CARDS GENERATED ---');
}

run().catch(console.error);
